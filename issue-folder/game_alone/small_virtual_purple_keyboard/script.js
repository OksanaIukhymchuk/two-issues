const Keyboard = {
    ...
    init() {
        ...
        toggleKeyboardButton.addEventListener("click", () => {
            ...
            isKeyboardOpen
                ? this.close()
                : this.open(this.onKeyInput, this.onKeyBackspace)
        });
                    // currentValue => textInput.value += currentValue,
                    // () => textInput.value = textInput.value.substring(0, textInput.value.length - 1)
                    // currentValue => textInputs.forEach(input => input.value += currentValue),
                    // () => textInputs.forEach(x => x.value = x.value.substring(0, x.value.length - 1))
                    // )
        // });
    },
    ...
    open(oninput, onbackspace, onclose) {
...
textInputs.forEach(x => x.addEventListener('focus', function () {
    let useKeyboardElements = document.querySelectorAll('.use-keyboard-input');
    useKeyboardElements.forEach(y => y.classList.remove('use-keyboard-input'));
    x.classList.add('use-keyboard-input');
}));
